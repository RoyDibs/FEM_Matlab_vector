...
figure(1);
subplot(2,1,1);
contourf(X, Y, zfemy, 'EdgeColor', 'none'); % Adjust the number of contour levels (20 in this example)
colormap(jet);
colorbar;
xlabel('x');
ylabel('y');
...
% Create a figure for the FEM solution
subplot(2,1,2);
contourf(X, Y, Z_exacty, 'EdgeColor', 'none'); % Adjust the number of contour levels (20 in this example)
colormap(jet);
colorbar;
xlabel('x');
ylabel('y');
title('Exact Solution (u_{y})');
...
figure(2);
subplot(2,1,1);
contourf(X, Y, zfemx, 'EdgeColor', 'none'); % Adjust the number of contour levels (20 in this example)
colormap(jet);
colorbar;
xlabel('x');
ylabel('y');
...
% Create a figure for the FEM solution
subplot(2,1,2);
contourf(X, Y, Z_exactx, 'EdgeColor', 'none'); % Adjust the number of contour levels (20 in this example)
colormap(jet);
colorbar;
xlabel('x');
ylabel('y');
title('Exact Solution (u_{x})');
...
l2_errory = sum(abs(u_exact(:,2)- (Z_fem(:,2))));
disp("L-2 error uy is: \n");
disp(l2_errory);
l2_errorx = norm(Z_exactx - zfemx);
disp("L-2 error ux is: \n");
disp(l2_errorx);
end